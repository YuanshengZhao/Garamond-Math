% !TeX document-id = {2e87e4fd-bd6e-4100-97ca-95b5e35fac9c}
...
\usepackage{fontspec}
\setmainfont{TeX Gyre Pagella}[Ligatures=TeX,Numbers={Lowercase,Monospaced}]
...
\NewDocumentCommand \defmathfont { m m m O{} }
{
  \newcounter{#1}
  \setcounter{#1}{-1}
  \setmathfont{#2}
    [
      version=#1,
      SizeFeatures={{Size=-10, Colour=999999}, {Size=10-, Colour=#3}},
      #4
    ]
  \fontspec_set_fontface:cNnn {#1} \x {ItalicFont={}, BoldFont={}, #4} {#2}
}
\ExplSyntaxOff

\defmathfont{lm}{latinmodern-math.otf}{CC6666}
\defmathfont{xits}{xits-math.otf}{CCCC66}
\defmathfont{termes}{texgyretermes-math.otf}{AA66CC}
\defmathfont{garamond}{Garamond-Math.otf}{66CCCC}[Path=../]
\defmathfont{fira}{FiraMath.otf}{6666CC}[Path=D:/LaTeX/FontTest/FiraMath-master/]
...
\item[T] \mathversion{termes} $\mathup{TeX\ Gyre\ Termes\ Math}$ (\ref{count:termes})
\item[G] \mathversion{garamond} $\mathup{Garamond\ Math}$ (\ref{count:garamond})
\item[F] \mathversion{fira} $\mathup{Fira\ Math}$ (\ref{count:fira})
\end{itemize}
...
\refstepcounter{termes}\label{count:termes}
\refstepcounter{garamond}\label{count:garamond}
\refstepcounter{fira}\label{count:fira}
