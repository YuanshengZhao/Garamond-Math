...
\setmathfont{Garamond-Math}[StylisticSet={1,7,8,9},Path=./]
...
\end{split}\]
\[A_A^AA_{AA}^{AA}A_x^x x_{AA}^{AA} x_A^A A_{xx}^{xx}\]
...
$K(x)T(x)\Theta(x)L(x)E(x)H(x)N(x)Z(x)O(x)A(x)P(x)Q(x)\symup H(x)\partial(x)\partial_x O_x\partial_T O_T\symup H_x\symup H_H$
