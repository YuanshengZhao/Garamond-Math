...
\newfontfamily\GaramondMath{Garamond-Math}[StylisticSet={1,7},Path=./]
% \setmathfont{LibertinusMath-Regular.otf}[Path=../libertinus/]
...
\NewDocumentCommand \BRACKETMAP { m m }
  { \[ \exp_args:Nx \tl_map_inline:nn {#2} { (#1{##1}) } \] }
\NewDocumentCommand \SUPSUBMAP { m m }
  { \[ \exp_args:Nx \tl_map_inline:nn {#2} {{ #1{##1 \sb{X}} }} \] }
...
\end{split}\]
\[ {a\over b} TT{}TLAPAVAMAWABA\rho_A\rho_{A{}}\rho_{AB}\rho_B\rho_x\rho_{T{}}\rho_0[x,y]\sin x[x,y]j[x,y]z[x,y]\sinh xH\upEta T_kT_{k_k}\]
\[[]\sinh\]
\[[]\mathrm s\]
\[[]s\]
$(0) T{ijT_oTT_x^xL_{ff}L_{jj}Uo}iUojj_x^xJjh(x)$

$K(x)T(x)\Theta(x)L(x)E(x)H(x)N(x)Z(x)O(x)A(x)P(x)Q(x)\symup H(x)$

$K[x]T[x]\Theta[x]L[x]E[x]H[x]N[x]Z[x]O[x]A[x]P[x]Q[x]\symup H[x]$
\[a_{ij}({{}j})ij TijT{ijT_oT_x^xL_{ff}L_{jj}Uo}iUojj_x^xJj[f][j][y](\symsf {fijk})\]
...
\begin{itemize}
  ...
  \item Superscript and subscript
        ...
        \[V_k^kW_k^k\Gamma_k^kT_k^k{\symup T}_k^k{\symup V}_k^kV_{\frac HH}\]
