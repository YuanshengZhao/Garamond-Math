...
%         \end{itemize}
  ...
  \item Brackets
  \[(jf)|jf\rangle(\ket{j})(\ket{k})\left((j)\right)\]
        \[
...
\begin{align*}
\int_{0}^aJ_0\left[\frac{x_n^{(0)}}{a}r\right]\left[\frac{x_m^{(0)}}{a}r\right]r\dd{r}=\frac{a^2}{2}J_1^2[x_n^{(0)}]\delta_m^n.\\
