...
\section{Alphabets}
       ...
       \[SO(3)SU(2)LAETSQ_AWOWCVO LX UUTTXAT afggffg\symbf{afggffg}\]
...
%         \end{itemize}
  ...
  \item Matrices:
        ...
        \]
  \item Nablas:
        \[ \rho\dot{\symbf v}=\mathscr{f}-\nabla p+\eta \symit{\nabla}^2 \symbf v+(\eta+\theta)\symbfup{\nabla}(\symbfit\nabla\cdot\symbf v)\]
        \[\bar{\psi}(\leftharpoonaccent{\nabla}+\rightharpoonaccent{\symbfit{\nabla}})\psi+\partial_\mu(\symup\partial^\mu\phi)-A_\lambda\epsilon^{\lambda\mu\nu}\symbf\partial_\mu(\symbfup\partial_\nu f)\]
...
\begin{split}	
\mathcal{F}^{-1}(\ket{j})
&{}=\frac{1}{\sqrt{2^n}}\sum_{k=0}^{2^n-1}\exp\left(-2\pi \ii \frac{jk}{2^n}\right)\ket{k}.\\
